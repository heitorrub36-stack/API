...
@Service
public class PassportFlowService {
    ...
    public void generateDefaultProcessSteps(Passport passport) {
        if (passport == null || passport.getId() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Passport must be saved before process step generation");
        }
    ...
    public PassportTask findFirstTaskByPassport(UUID passportId) {
        return taskRepository.findByActivityPassportId(passportId).stream()
                .findFirst()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "No task found for this passport step structure"));
    }
